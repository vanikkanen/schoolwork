...
bool update_file(const std::string file_name, std::map< std::string, Student* >& alphabetical_order)
{
        ...
        if(command == "U" or command == "u") {
            ...
            if(parts.size() != 2){
                ...
                if (student_numbers.find(parts.at(1)) == student_numbers.end()){
                    ...
                    change_number(*(student_numbers.at(parts.at(1))));
                    if(not update_file(file_name, user_ids)) {
                        std::cout << "Error in reading file!" << std::endl;
                        return EXIT_FAILURE;
                    }
                }
