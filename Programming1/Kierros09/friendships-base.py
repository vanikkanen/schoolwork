...
StudentId: 282688, 284930
Firstname Lastname: Valtteri Nikkanen, Atte Harmoinen
Email: valtteri.nikkanen@tuni.fi, atte.harmoinen@tuni.fi
"""
...
def print_command(network, namelist):
    ...
    for name in sorted(network):
        print(name)
        for friend in sorted(network[name].split(",")):
            print("-", friend)
...
def friends_command(network, namelist):
    ...
    try:
        ...
        for name in sorted(friends.split(",")):
            print(name)
...
def common_friends_command(network, namelist):
    ...
    friends1 = set(network[name1].split(","))
    friends2 = set(network[name2].split(","))

    common_friends = friends1 & friends2
    if common_friends == set():
